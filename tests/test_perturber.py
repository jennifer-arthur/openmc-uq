...
@pytest.mark.parametrize("param, value, ptype", [
    ("nonexistent.r", 0.01, "geometry"),
    ("nonexistent.density", 0.01, "density"),
    ("nonexistent.nonexistent", 0.01, "isotopic"),
])
def test_nonexistent_name(perturber, param, value, ptype):
    with pytest.raises(KeyError):
        perturber.perturb(param, value, ptype)


@pytest.mark.parametrize("param, value, ptype", [
    ("sph10r", 0.01, "geometry"),
    ("HEU6..density", 0.01, "density"),
    ("HEU6..U235", 4e-4, "isotopic"),
])
def test_missing_dot(perturber, param, value, ptype):
...
@pytest.mark.parametrize("param, value, ptype", [
    ("HEU6.density", 0.001, "density"),
    ("HEU6.U235", 0.01e-4, "isotopic"),
])
def test_double_perturb(perturber, param, value, ptype):
    perturber.perturb(param, value, ptype)
...
def test_isotopic_perturb(perturber, isotope, delta):
    param = f"HEU6.{isotope}"
    name, _ = param.split('.')
    material = perturber.materials[name]
...
def test_isotopic_restore_fraction_type(perturber, param, value):
    name, isotope = param.split('.')
    material = perturber.materials[name]
    nominal_type = perturber._get_fraction_type(material)

    perturber.perturb(param, value, "isotopic")
...
def test_isotopic_restore_order(perturber, param, value):
    name, isotope = param.split('.')
    material = perturber.materials[name]
...
def test_isotopic_restore_exact_float(perturber, param, value):
...
def test_isotopic_locality(perturber):
    param_a, delta_a = "HEU6.U235", 0.1e-3
    param_b, delta_b = "HEU5.U235", 4.4e-4
